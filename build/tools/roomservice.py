...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from AfterLife Github (http://github.com/AfterLife)." % device)
...
def is_in_manifest(projectpath):
    ...
    try:
        lm = ElementTree.parse(".repo/manifests/afterlife.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        print('Adding dependency: AfterLife/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "AfterLife/%s" % repo_name,
...
print("Repository for %s not found in the AfterLife Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
