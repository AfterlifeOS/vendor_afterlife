...
- afterlifegerrit:   A Git wrapper that fetches/pushes patch from/to AfterLife Gerrit Review.
- afterliferebase:   Rebase a Gerrit change and push it again.
- afterliferemote:   Add git remote for AfterLife Gerrit Review.
...
function afterliferemote()
{
    ...
    local AFTERLIFE_USER=$(git config --get review.review.afterlife.org.username)
    if [ -z "$AFTERLIFE_USER" ]
    then
        git remote add afterlife ssh://review.afterlife.org:29418/$PFX$PROJECT
    else
        git remote add afterlife ssh://$AFTERLIFE_USER@review.afterlife.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/AfterLifePrjkt13/$PROJECT
...
function afterlifegerrit() {
    ...
    local user=`git config --get review.review.afterlife.org.username`
...
function afterliferebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AfterLife Gerrit Rebase Usage: "
    ...
    git fetch "http://review.afterlife.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
